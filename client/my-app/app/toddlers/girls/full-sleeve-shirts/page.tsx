...
export default function CategoryPage() {
  const filteredProducts = products.filter(
    (item) =>
      ...
      item.gender === "girl" &&
      item.tags.includes("full-sleeve-shirts")
  );
  ...
  return (
    <section className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-10 sm:py-14">
      <div>
        ...
        >
          Girls Toddlers - Full Sleeve Shirts
        </h1>
        <p className="mt-1 text-sm" style={{ color: "var(--muted)", fontFamily: "'DM Sans', sans-serif" }}>
          Demo listing for Girls Toddlers in Full Sleeve Shirts.
        </p>
      </div>

      <div className="mt-8 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
        {filteredProducts.map((product) => (
          <Link key={product.id} href={"/toddlers/girls/full-sleeve-shirts/" + product.id} className="block cursor-pointer group">
          <article
